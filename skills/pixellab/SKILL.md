...
- **검색 인덱스 의존(최초 1회 setup)**: `find`/`add` 는 SQLite FTS5 파생 인덱스(`better-sqlite3`)로 후보를 추린다(판정 점수·0.6 임계값은 기존 `score()` 가 그대로 소유 — 의미 불변). 백엔드를 못 쓰면 CLI 는 "setup 실행" 안내와 함께 비정상 종료한다. 최초 1회 `node "${CLAUDE_PLUGIN_ROOT}/scripts/pixellab-cache.mjs" setup` 으로 설치한다(자동 npm install 은 setup 에서만; find/add/훅은 자동설치 안 함). **Node 메이저 버전을 올린 뒤**에는 패키지가 그대로여도 네이티브 바인딩(ABI)이 안 맞아 못 쓰는 상태가 되는데, 이때 `npm install` 은 아무 일도 하지 않는다 — `setup` 이 이를 감지해 `npm rebuild better-sqlite3` 로 자동 복구한다(수동 복구도 같은 명령). 파생 인덱스 `<root>/index.sqlite` 는 gitignore·재빌드 가능하며 `index.json`+PNG 원본이 진실의 원천. 부재/손상/`index.json` 변경 시 자동 rebuild, 수동 재구성은 `rebuild`.
...
- **방향은 후처리로 만들 수 있는 것을 생성하지 않는다**(사용자 상시 지시, 2026-08-20): 좌우 대칭 방향은 **코드/이미지 후처리의 수평 플립**으로 무료로 만든다 — 동↔서, 남동↔남서, 북동↔북서. 따라서 8방향이 필요해도 **5방향(남·북·동·남동·북동)만 생성**, 4방향이면 **3방향(남·북·동)만 생성**하고 나머지는 플립. 애니메이션도 같은 원리로 플립 가능한 방향은 생성하지 않는다. 단, 비대칭 디테일(한손 무기·방패 등)이 플립 시 좌우가 바뀌는 것은 게임 관례상 허용 — 사용자가 명시적으로 문제 삼을 때만 해당 방향을 실제 생성으로 교체한다.
